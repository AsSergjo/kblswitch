...
> Версия **2.0.0.0** · C++17 · Windows 10/11 · x64
...
1. Скачайте последний релиз или скомпилируйте сами (см. «Сборка»).
2. Разместите `kblswitch.exe` в любой папке (вместе с папкой `language\`).
...
В репозитории есть GitHub Actions workflow (`.github/workflows/build.yml`): собирает проект на `windows-latest` и выкладывает `kblswitch.exe` артефактом.
...
kblswitch.cpp          Ядро: главное окно, жизненный цикл, диспетчеризация
...
resource.rc/.h         Ресурсы: иконка, версия 2.0.0.0
...
- Приложение использует **низкоуровневый хук клавиатуры** (`WH_KEYBOARD_LL`) и может эмулировать нажатия (`SendInput`/`keybd_event`) — это необходимо для работы. Игнорирует **инжектированные** события (`LLKHF_INJECTED`), чтобы не создавать зацикливание.
